    ...
    mclOutputHandlerFcn print_handler)
{
    ...
    if (!mclmcrInitialize())
        return false;
    if (!GetModuleFileName(GetModuleHandle("draw"), path_to_dll, _MAX_PATH))
        return false;
...
{
    return drawInitializeWithHandlers(mclDefaultErrorHandler, mclDefaultPrintHandler);
}
...
{
    return mclFeval(_mcr_inst, "draw", nlhs, plhs, nrhs, prhs);
}

LIB_draw_CPP_API 
void MW_CALL_CONV draw(const mwArray& Z)
{
    mclcppMlfFeval(_mcr_inst, "draw", 0, 0, 1, &Z);
}
